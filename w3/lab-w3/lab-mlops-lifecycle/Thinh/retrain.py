...
Flow:
  ...
  3. Train new model on sliding-window data (baseline + drift) via pipeline.py logic
  4. Validate v2 on holdout (old pattern) — v2 must not be worse than v1
  5. Register new version with alias 'staging'
  6. Print approval prompt — wait for human input
  7. On approval: promote 'staging' → 'production', call serve.py /reload
  8. Post-deploy monitor: auto-rollback if v2 degrades on eval set
  9. Log full decision trail to MLflow run (parameters + metrics + tags)
...
# ---------------------------------------------------------------------------
# Post-deploy monitoring & auto-rollback (Stress 3)
# ---------------------------------------------------------------------------
...
def main():
    ...
    combined_df = pd.concat(
        [baseline_df_train, drift_df_train], ignore_index=True
    )
    print(
