...
def query_prometheus(prometheus_url: str, promql: str) -> float | None:
    ...
    return None
def verify_service(prometheus_url, service, baseline, timeout_s, poll_interval_s, min_samples, alertname=""):
    ...
    while time.time() < deadline:
        ...
        ok = up_ok if alertname == "InstanceDown" else (latency_ok and up_ok)
        log.info("VERIFY_SAMPLE", service=service, sample=samples, latency_p99_ms=latency, up=up, latency_ok=latency_ok, up_ok=up_ok)
        if ok:
            passes += 1
